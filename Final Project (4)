...
//use a for-loop to iterate through the deck and call the draw method
		for(int i = 0; i < 52; i++) {
			if (i % 2 == 0) {
...
//use a for-loop to have each player draw a card
		for(int j = 0; j < 26; j++) {
			Card card1 = new Card("firstCard", 0);
				 card1 = player1.flip(card1);
			System.out.println(player1.getName() + " played The ");
				card1.describeCard();
				System.out.print(" which is worth " + card1.getValue());
			Card card2 = new Card("secondCard", 0);
			player2.flip(card2);
			System.out.println(player2.getName() + " played The ");
			card2.describeCard();
			System.out.print(" which is worth " + card2.getValue());
			System.out.println();
